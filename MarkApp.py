...
def adminpage():
    ...
    def call(vale):
        try:
            marks = fetchmarks(selectmark.get())
    ...
    selectmark=customtkinter.CTkOptionMenu(frame1,width=310,font=('Lexend medium', 15),corner_radius=25,fg_color='#8f00ff',button_color='#6303ac',button_hover_color='#4F0260',values=options,dropdown_font=('Lexend ',12),command=call)
    selectmark.place(x=30, y=65)
    selectmark.set('Select Student')

    def save():
        update(s1entry.get(),s2entry.get(),s3entry.get(),s4entry.get(),selectmark.get())
        update_rank()
...
def dashboardstudentno():
    ...
    rank = Label(frame, text='Your Result Has ', font=('Lexend medium', 20), fg='#fff', bg='#8f00ff')
    yourrank = Label(frame, text='Not Been Published Yet', font=('Lexend Medium', 20), fg='#fff', bg='#8f00ff',)
    # Frame(frame,width=250,height=2,bg='#fff').place(x=5,y=50)
    # yourrank2 = Label(frame, text='yet', font=('Lexend Medium', 20), fg='#fff', bg='#8f00ff',)

    rank.place(x=39, y=50)
    yourrank.place(x=39, y=85)  
    ...
    sub1=customtkinter.CTkLabel(master=tabview.tab('NA'),text='Please contact your Teacher',font=('Lexend Medium',20))
    sub1.place(x=20,y=-10)
    home.mainloop()
homepage()